"""Horizontal demo's logic: cycling main_alignment via self.app.reload(...), and
...
class HorizontalDemo(ViewModel):
    """State and commands for `Horizontal_View.yaml`.

    `main_alignment` is a plain `style.*` property, not one of the handful
    of templated fields (`text:`, `value:`, ...), so it cannot be bound to
    a Signal with `{{ }}`. The button instead reloads `Horizontal_View.yaml`
    ...
    def __init__(self) -> None:
        ...
        self.view_source = (Path(__file__).parent / "Horizontal_View.yaml").read_text()
    ...
    def _view(self) -> dict[str, Any]:
        ...
        live_demo["style"]["main_alignment"] = alignment
        label = _find(doc, "label")
        assert label is not None
        label["text"] = f"main_alignment: {alignment}"
        return doc
